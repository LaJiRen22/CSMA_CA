...
void Node::initialize()
{
    ...
    this->getDisplayString().setTagArg("1" ,1, "black");
...
void Node::finish()
{
    ...
    cout<<"The total sent data packets are"<< totalSentDataPackets<< endl;
...
void Node::networkInitialization()
{
    ...
    //已发送包个数
    this->lossPackets = 0;
    //丢包个数，当一个数据包发送次数超过retryLimit时会将该包丢弃。
...
dataPacketGeneration();//节点服从泊松分布产生數据包，也就是相邻两包间隔服从指数分布
this->destinationNode = getRandomDestination(this->getId()); //set CSMAEvent
...
//cout<< "Node”<< this->getId() << ”jin dataPacketGeneration le"<< endl ;
//data packet generation poisson arrival
    if(this->dataQueue.empty() || (this->dataQueue.size() < this->queueSize)){
        ...
        data->setDestination_id(getRandomDestination(this->getId())->getId());
...
//cout << "Node ”<< this->getId() << "jin dataPacketAccess le"<< endl;
    Busy = 0;
    IsDataQueueEmpty();
}

void Node::DIFScheck(){
//cout<<"Node" <<this->getId()<<" ,DIFScheck-- Busy"<<Busy<<end1;
//cout << "Node ”《< this->getId() << "jin DIFScheck 1e"《< endl;
    ...
    if(DIFS_counter == 5){
            ...
            if(first_time_data_transmission == 0){
                ...
                if(CW_counter == -1){
                    ...
                    //cout<<"CW_ couter is ====== "<<CW_ counter<<endl;
                }
                ...
                //cout << "Node ”<< this->getId() << "DIFS busy"<< endl;
                DIFS_counter = 0;
...
void Node :: CWcheck(){
//cout<<"Node"<cthis->getId()<",CWcheck--Busyuumuu"<<Busyc<endl;
//cout << "Node<< this->getId() << "jin CWcheck le" << endl;
if(Busy ==0& CW_counter != 0){
    ...
    if(CW_counter != 0){
       ...
       //cout <《< "Node”<< this->getId() << "CW counter is”<< CW_ counter <《< endl;
       DIFS_CheckEvent->setKind(EV_DIFSCHECK);
